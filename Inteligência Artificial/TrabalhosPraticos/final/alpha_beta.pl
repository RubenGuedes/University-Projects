...
game_ouri(State, 0, Timer) :-
    ...
    Act is 6 - Action,
    writef("O computador jogou na posição '%w'\n", [Act]), % Imprime no terminal a coluna que o computador jogou
...
game_ouri(State, 1, Timer) :-
    print_board(State), % Imprime o tabuleiro
...
move(Action) :-
  ...
  0 -> Sem jogada possível (O jogo termina)
  1 -> Jogar na coluna 1 (Mais à esquerda)
  2 -> Jogar na coluna 2
  3 -> Jogar na coluna 3
  4 -> Jogar na coluna 4 
  5 -> Jogar na coluna 5 
  6 -> Jogar na coluna 6 (Mais à direita)"
  ),
  read(Option),
